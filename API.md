		...
		if user is not None:
			if user.password == password.contents.str:
				...
				http_cerver_auth_jwt_add_value_int (
					http_jwt, "iat".encode ('utf-8'), int (time.time ())
				)
				http_cerver_auth_jwt_add_value (
					http_jwt, "id".encode ('utf-8'), user.id.encode ('utf-8')
				)
				http_cerver_auth_jwt_add_value (
					http_jwt, "name".encode ('utf-8'), user.name
				)
				http_cerver_auth_jwt_add_value (
					http_jwt, "username".encode ('utf-8'), user.username
				)
				http_cerver_auth_jwt_add_value (
					http_jwt, "role".encode ('utf-8'), user.role.encode ('utf-8')
				)
...
def start ():
	...
	users_route = http_route_create (
		REQUEST_METHOD_GET, "api/users".encode ('utf-8'), main_users_handler
	)
	...
	users_register_route = http_route_create (
		REQUEST_METHOD_POST, "register".encode ('utf-8'), users_register_handler
	)
	...
	users_login_route = http_route_create (
		REQUEST_METHOD_POST, "login".encode ('utf-8'), users_login_handler
	)
	...
	users_profile_route = http_route_create (
		REQUEST_METHOD_GET, "profile".encode ('utf-8'), users_profile_handler
	)
